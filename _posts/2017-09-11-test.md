...
- 1 Sprint の中で Sprint Planning は 2 段階に分けて行い、一度目の Sprint Planning（上記図中の Sprint Planning 1）には、必ず全ての Team（もしくはその代表者）が参加する
...
LeSS がこのような形態を取っている主な理由については、次の章の [プロダクトの定義](#definition-of-product) で説明する。

<a name="definition-of-product"></a>
## プロダクトの定義

LeSS における「プロダクト」の定義とは何か？Less では一つの「プロダクト」を可能な限り **大きく** 定義するのが良いとされている。

具体例を出してみよう。例えば、ある Web サービス（Web Service X）を開発していて、そのサービスを展開するプラットフォームが Web ブラウザ、iOS, Android といったように複数存在するとする。この場合にプロダクトを

- Web Service X (Web)
- Web Service X (iOS)
- Web Service X (Android)

のように別々に定義するのではなく、一つの Web Service X として定義することを、この場では「プロダクト」をより大きく定義すると呼んでいる。
